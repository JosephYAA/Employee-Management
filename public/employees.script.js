...
async function getDataByName() {
  ...
  if (name) {
    try {
      // const res = await fetch(`${baseURL}/employees?title=${title}`);
...
async function putData() {
  ...
  const surname = document.getElementById("put-surname").value;
  const department = document.getElementById("put-department").value;
...
async function deleteAllData() {
  ...
  try {
    const res = await fetch(`${baseURL}/employees`, { method: "delete" });
